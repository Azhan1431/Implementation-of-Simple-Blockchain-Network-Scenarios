...
class Transaction(object):
    def __init__(self, sender, recipient, data, timestamp, id='', sig='', 
                 tx_type='normal', vote_id=None, option_id=None, voters=None):
        """
        交易的初始化
        :param sender: 发送方的地址
        ...
        :param tx_type: 交易类型(normal/vote)
        :param vote_id: 投票ID(当tx_type=vote时)
        :param option_id: 选项ID(当tx_type=vote时)
        :param voters: 已投票用户列表(防重复投票)
        """
...
class Vote(BaseModel):
    ...
    def validate_vote(self, tx: Transaction) -> bool:
        """验证投票交易有效性"""
        # 检查投票是否过期
        ...
        # 检查选项是否存在
        option_exists = any(opt.option_id == tx.option_id for opt in self.options)
        if not option_exists:
            return False
            
        # 检查是否重复投票
        if tx.sender in tx.voters:
            return False
            
        return True

    def count_votes(self, transactions: List[Transaction]):
        """统计投票结果"""
        for tx in transactions:
            if tx.tx_type == 'vote' and tx.vote_id == self.vote_id:
                if self.validate_vote(tx):
                    for option in self.options:
                        if option.option_id == tx.option_id:
                            option.vote_count += 1
                    tx.voters.append(tx.sender)
