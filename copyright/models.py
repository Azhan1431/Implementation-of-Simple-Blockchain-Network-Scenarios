...
class Transaction(object):
    def __init__(self, sender, recipient, data, timestamp, id='', sig='', work_hash='', author='', work_type='', description='', license_type='none', allowed_users=None, revenue_split=None):
        """
        交易的初始化
        :param sender: 发送方的地址
        ...
        :param work_hash: 作品哈希值(版权登记用)
        :param author: 作者信息(版权登记用)
        :param work_type: 作品类型(版权登记用)
        :param description: 作品描述(版权登记用)
        :param license_type: 授权类型(none/free/restricted)
        :param allowed_users: 允许使用的用户列表
        :param revenue_split: 收益分成比例{author:比例, platform:比例}
        """
        self.sender = sender
        ...
        self.revenue_split = revenue_split or {'author': 0.7, 'platform': 0.3}
    ...
    def to_string(self):
        ...
        return f'{self.sender}{self.recipient}{self.data}' \
               ...
               f'{self.work_hash}{self.author}{self.work_type}{self.description}' \
               f'{self.license_type}{",".join(self.allowed_users)}' \
               f'{self.revenue_split["author"]}{self.revenue_split["platform"]}'
